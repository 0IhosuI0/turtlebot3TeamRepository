...
*   **`__init__(self)` 함수:**
    ...
    *   **세부 설명:**
        ...
        *   **장애물 회피 변수 초기화:**
            *   `self.safety_distance`: 장애물 회피를 위한 안전 거리를 설정합니다 (0.7m로 조정됨).
            *   `self.critical_safety_distance`: 매우 가까운 장애물에 대한 비상 후진을 위한 임계 안전 거리를 설정합니다 (0.3m).
...
*   **`scan_callback(self, msg)` 함수:**
    ...
    *   **세부 설명:**
        ...
        *   `self.lidar_zones` 딕셔너리에 전방(330-360도 및 0-30도), 좌측(30-90도), 우측(270-330도) 영역의 최소 거리를 계산하여 저장합니다. 0.01보다 큰 유효한 거리 값만 고려합니다. (더 넓은 LiDAR 영역을 사용하여 장애물 감지 개선)
...
*   **`generate_motion_command(self, target_box, frame_width)` 함수:**
    ...
    *   **세부 설명:**
        *   **0. 치명적 장애물 회피 계층 (최고 우선순위 - 후진):**
            *   `self.critical_safety_distance` (0.3m) 이내에 장애물이 감지되면, 로봇은 즉시 후진합니다. 이는 매우 가까운 장애물에 대한 비상 회피 동작입니다.
        *   **1. 장애물 회피 계층 (높은 우선순위 - 전방):**
        ...
        *   **2. 상태 기반 동작 계층:**
            *   로봇의 `robot_state`에 따라 다른 동작을 수행합니다.
                ...
                *   `IDLE` 상태: 로봇을 정지시킵니다.
        *   **3. 반응형 측면 장애물 회피 계층:**
            *   `self.lidar_zones['left']` 또는 `self.lidar_zones['right']`가 `self.safety_distance`보다 작으면, 즉 측면에 장애물이 있으면 로봇의 전진 속도를 줄이고 장애물 반대 방향으로 회전하는 각속도를 추가하여 회피합니다. (각속도 조정값 0.9에서 0.7로 변경)
            *   측면 장애물이 감지되면, 전진 속도를 `0.1`로 제한하거나 후진 속도를 `-0.1`로 제한하여 기동성을 유지합니다.
...
*   **`get_following_twist(self, target_box, frame_width)` 함수:**
    ...
    *   **세부 설명:**
        ...
        *   `error_x`에 비례하여 각속도(`angular.z`)를 설정하여 대상을 이미지 중앙에 오도록 로봇을 회전시킵니다. (0.002에서 0.003으로 증가)
        *   대상 바운딩 박스의 면적(`target_area`)을 계산하여 대상과의 거리를 추정합니다.
            *   면적이 작으면 (대상이 멀리 있으면) 전진 속도(`linear.x`)를 양수로 설정하여 다가갑니다. (0.15로 증가)
            *   면적이 크면 (대상이 가까이 있으면) 전진 속도를 음수로 설정하여 뒤로 물러납니다. (-0.15로 증가)
...
*   **`get_searching_twist(self)` 함수:**
    ...
    *   **세부 설명:**
        ...
        *   **탐색 상태 (`search_state`):**
            ...
            *   `SWEEP` 상태: `PEEK` 상태에서 대상을 찾지 못하면, 좌우로 번갈아 가며 넓게 회전하여 대상을 탐색합니다 (3.0초마다 방향 전환, 각속도 0.4에서 0.6으로 증가).
